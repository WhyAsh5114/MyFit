...
<li class="flex flex-col bg-primary w-full h-fit rounded-md p-2">
  <div class="flex gap-2">
    <p class="font-semibold grow" data-testid="exercise{idx + 1}-name">{exercise.name}</p>
  ...
  {#if exercise.note}
    <p
      class="bg-info bg-opacity-75 text-black px-1 text-sm rounded-sm mt-1.5"
      data-testid="exercise{idx + 1}-note"
    >
      {exercise.note}
    </p>
  {/if}
