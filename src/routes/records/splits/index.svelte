...
<div class="flex flex-col w-full max-w-md gap-2">
    {#each splits as split, i}
        <div
            class="flex flex-col gap-1 bg-primary rounded-lg p-3 active:scale-95 hover:bg-opacity-50 transition-all border-2 {split.name === user.activeSplit ? 'border-accent' : 'border-base-100'}"
            in:scale
        >
