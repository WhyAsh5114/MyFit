...
<script lang="ts">
    ...
    $: updateChanges(splitName, frequency, progressionValue, thisActive);
    ...
    function editWorkout(index: number) {}
</script>
            ...
            <div class="flex flex-col gap-2 flex-grow justify-center text-base">
                {#each days as day, i}
                    <div class="flex text-black">
                        ...
                        <input class="w-full text-center py-1" value={split.schedule[i]} />
                        <button
                            class="basis-12 text-center flex-shrink-0 bg-base-100 text-white rounded-r-lg py-1 hover:brightness-75"
                            on:click={() => editWorkout(i)}
                        >
                            {i in uniqueWorkoutsIndices ? 'Edit' : ''}
                        </button>
                    </div>
                {/each}
            </div>
        ...
        <div class="flex flex-col gap-2 md:gap-4">
            ...
            <div class="stat bg-primary rounded-xl">
                ...
                <div class="stat-value text-accent">
                    {new Date(split.timeCreated).toJSON().slice(0, 10)}
                </div>
            </div>
        </div>
    </div>
